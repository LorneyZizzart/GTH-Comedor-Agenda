...
public class C_TipoComensalModel {
    ...
    public List<C_TipoComensal> getAllTipoComensal() {
        ...
        try {
            ...
            db.conectar();
            String sql = "SELECT Id, Nombre, MinimiDias, Estado, fechaRegistro, UsuarioCrea, fechaActualizacion, diaInicio, descuentoDesayuno, descuentoAlmuerzo, descuentoCena"
                        + " FROM CTipoComensal "
                        + " WHERE Estado <> 6";
    ...
    public C_TipoComensal getTipoComensalById(int idTipoComensal) {
        ...
        try {
            ConectaSqlServer db = new ConectaSqlServer();
            db.conectar();
            String sql = "SELECT Id, Nombre, MinimiDias, Estado, UsuarioCrea, fechaRegistro, fechaActualizacion, diaInicio, descuentoDesayuno, descuentoAlmuerzo, descuentoCena "
                        + " FROM CTipoComensal "
    ...
    private String SaveTipoComensal(C_TipoComensal tipoComensal) {
        ...
        try {
            ...
            String sql = "INSERT INTO CTipoComensal\n"
                    + "           (Nombre, MinimiDias, Estado, fechaRegistro, UsuarioCrea, fechaActualizacion, diaInicio, descuentoDesayuno, descuentoAlmuerzo, descuentoCena)\n"
                    + "     VALUES\n"
                    + "           ('" + tipoComensal.getNombreComensal()+ "',"+ tipoComensal.getMinimoDias()+","  + tipoComensal.getEstado() + ", CONVERT(date, SYSDATETIME()), " + tipoComensal.getUsuarioCrea() + ", CONVERT(date, SYSDATETIME()),"+tipoComensal.getDiaInicio()+",'"+tipoComensal.getDescuentoDesayuno()+"','"+tipoComensal.getDescuentoAlmuerzo()+"','"+tipoComensal.getDescuentoCena()+"')";
            db.insertar(sql);
            db.cierraConexion();
        } catch (SQLException e) {
